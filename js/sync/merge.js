...
/** A value a side actually holds. An empty string is a default, not an edit:
 * ensureDay creates `notes: ''`, and stamping that as new would let the first
 * tick on one device erase a note typed on another (audit F01, 2026-09-19). */
const heldValue = (v) => v !== undefined && v !== null && v !== '';
...
function stampDayPaths(s, key, prevFp, val, t) {
  ...
  const fresh = prevFp === undefined;        // the day record did not exist yet
  const dp = (s.dp[key] ||= {});
  const paths = new Set([...dayPaths(prev), ...dayPaths(val)]);
  for (const path of paths) {
    const now = getPath(val, path);
    if (fresh && !heldValue(now)) continue;  // an empty default is not an edit
    if (fingerprint(getPath(prev, path)) !== fingerprint(now)) {
 ...
 * A document that already syncs gets 1 for its gaps, not now: an unstamped
 * record is something written around update(), and stamping it as the newest
 * thing in the document would let it beat a real edit elsewhere (audit F04). */
export function stampAll(doc, deviceId, now = Date.now()) {
  const s = ensureSync(doc, deviceId);
  const fresh = Object.keys(s.rec).length === 0;
  for (const key of collectRecords(doc).keys()) {
    if (s.rec[key] === undefined) s.rec[key] = fresh ? now : 1;
  }
  return doc;
}
...
function mergeDay(lVal, lT, lDp, rVal, rT, rDp) {
  ...
  for (const path of paths) {
    ...
    // An empty value with no stamp of its own is a default, not a deletion
    // (audit F01): it must not outrank the other side's real one.
    const lp = lDp?.[path] ?? (heldValue(lv) ? lT : -1);
    const rp = rDp?.[path] ?? (heldValue(rv) ? rT : -1);
    let take = lv;
    if (fingerprint(lv) !== fingerprint(rv)) {
      take = winner(lp, heldValue(lv), lv, rp, heldValue(rv), rv) === 'a' ? lv : rv;
    }
  ...
  // The stamp has to say "newer than the copy this does not match", or the
  // merged day can never travel. A day resolved in favour of THIS device, on a
  // record the other side had stamped later, used to keep the older stamp: the
  // push step sends a record only when its stamp beats the remote's, so the
  // day sat on the device for good and the chip read "1 to sync" with nothing
  // it could do (his iPhone on d|2026-09-14, 2026-09-19).
  const stamp = same(rVal) ? rT : same(lVal) && lT > rT ? lT : Math.max(lT, rT) + 1;
...
export function mergeDocs(local, remote, { now = Date.now() } = {}) {
  ...
  for (const key of keys) {
    ...
    if (!lLive && lDel === undefined) {
      if (rLive) {
        putRecord(out, key, rRec.get(key)); ls.rec[key] = rs.rec[key] ?? 1;
  ...
  for (const [k, t] of Object.entries(ls.del)) {
    const rt = rs.del?.[k];
    if (rt === undefined ? (rRec.has(k) || rs.rec[k] !== undefined) : t > rt) pushed++;
    // A newer tombstone than the remote's is owed too: pendingKeys counts it,
    // and uncounted here it was never pushed, so "1 to sync" never cleared
    // (2026-09-22 audit).
  }
 ...
 *
 * Only what a sync could actually send counts. A stamp whose record is no
 * longer in the document cannot be pushed (mergeDocs sends live records and
 * tombstones), so counting one left a device saying "1 to sync" for good, with
 * nothing to do about it (his report, 2026-09-19; the same shape as the orphan
 * tombstone in audit F37).
 */
export function pendingKeys(doc, since) {
  const s = doc?._sync;
  const out = [];
  if (!s) return out;
  const live = collectRecords(doc);
  if (isAck(since)) {
    for (const [k, t] of Object.entries(s.rec || {})) if (since.rec[k] !== t && live.has(k)) out.push(k);
    // A tombstone for a record the remote never held is nothing to send, and
    // counting it left a device permanently "1 to sync" (audit F37).
    for (const [k, t] of Object.entries(s.del || {})) {
      if (since.del[k] === t) continue;
      if (since.rec[k] === undefined && since.del[k] === undefined) continue;
      out.push(k);
    }
    return out;
  }
  const cursor = Number(since) || 0;   // older builds: a single time cursor
  for (const [k, t] of Object.entries(s.rec || {})) if (t > cursor && live.has(k)) out.push(k);
