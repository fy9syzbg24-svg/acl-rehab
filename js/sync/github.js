...
/**
 * The file's text. Over 1 MB the contents API sends no content (encoding
 * "none"), so the same file is fetched again as raw text: the synced record
 * only grows, and without this every sync would fail once it passed 1 MB
 * (2026-09-22 audit). The sha still comes from the first answer.
 */
async function textOf(conn, json) {
  if (json.content && json.encoding !== 'none') return fromBase64(json.content);
  if (!(json.size > 0)) return fromBase64(json.content || '');
  const h = { ...headers(conn.token), Accept: 'application/vnd.github.raw+json' };
  const res = await fetch(fileUrl(conn), { headers: h, cache: 'no-store' });
...
export async function ghGetFile(conn) {
  ...
  const text = await textOf(conn, json);
  let doc = null;
  try {
    doc = JSON.parse(text);
  } catch (e) {
...
/**
 * Read any other file in the same private repo, read only. Used for the derived
 * layers that are not part of the record (the ring summary), so they reach the
 * phone and iPad without entering the synced document.
 * Returns { doc, sha }, or { doc: null } when the file does not exist yet.
 */
export async function ghGetPath(conn) {
  const res = await fetch(fileUrl(conn), { headers: headers(conn.token), cache: 'no-store' });
  if (res.status === 404) return { doc: null, sha: null };
  if (!res.ok) throw new GitHubError(res.status, await res.text());
  const json = await res.json();
  const text = await textOf(conn, json);
  try {
    return { doc: JSON.parse(text), sha: json.sha };
  } catch (e) {
    throw new GitHubError(200, `${conn.path} is not valid JSON: ${e}`);
  }
}
