...
//   p|<sub>|<key>             program.stage / .band / .weeklyTarget / .days / .clinicDays / .mins / .timer / .schedule / .seen / .ownSteps
//   x|<sub>|<key>             pk.drugs / .shots / .feel / .plans / .pens (the medication level tab)
...
// pushSubs holds one push subscription per device, keyed by device id, so the
// Mac's sender knows where to post a reminder for a phone it can never reach
// directly (the phone talks to GitHub, not to 127.0.0.1). It is registered here
// for the same reason caseFile is: a device that did not have it would
// otherwise push a document without it and wipe every other device's.
// settings is a key map, so each setting is its own record. That means
// settings.remind (which reminders he wants, and his quiet hours) syncs as one
// record with no registration of its own: the switch is tapped on the phone and
// the sender on the Mac reads it from the relay.
const KEY_MAPS = { s: 'settings', g: 'planGoals', f: 'planFocus', k: 'caseFile', w: 'pushSubs' };
...
// ownSteps (2026-09-23) holds his own progression steps per program item, an
// array of { t, at } read after the clinicians' steps (js/progressions.js).
// Unregistered, a device without it would push a document lacking it and wipe
// every step he added.
// pk (2026-09-19) is the medication level tab: the drug's model and sources
// (drugs), each logged shot (shots), the daily cravings check (feel, keyed by
// date), the schedules he compares (plans) and his pens and prices (pens). All
// of it is his data; the shell only holds the arithmetic (js/pk.js).
const SUB_MAPS = { b: ['melbourne', ['phases', 'measures']], p: ['program', ['stage', 'band', 'weeklyTarget', 'days', 'clinicDays', 'mins', 'timer', 'schedule', 'seen', 'ownSteps']],
                   x: ['pk', ['drugs', 'shots', 'feel', 'plans', 'pens']] };
...
export function collectRecords(doc) {
  ...
  // A key holding undefined is not a record: JSON drops it on every save, so
  // counting it as live turned a removal into an "edit" no tombstone ever
  // carried (2026-09-22 audit, the plan focus untick).
  for (const [tag, name] of Object.entries(KEY_MAPS)) {
    for (const [k, v] of Object.entries(doc[name] || {})) if (v !== undefined) out.set(`${tag}|${enc(k)}`, v);
  }
  for (const [tag, [parent, subs]] of Object.entries(SUB_MAPS)) {
    for (const sub of subs) {
      for (const [k, v] of Object.entries((doc[parent] || {})[sub] || {})) {
        if (v !== undefined) out.set(`${tag}|${sub}|${enc(k)}`, v);
      }
    }
  }
