...
export function planStreak(doc, today) {
  ...
  for (let guard = 0; guard < 2000; guard++) {
    ...
    if (ids.length) {
      const entries = doc.days?.[d]?.entries || [];
      const done = ids.every((id) => !BY_ID[id] || itemStatus(BY_ID[id], entries).state === 'done');
      if (done) n++;
...
export function dayComplete(doc, iso) {
  ...
  const done = ids.filter((id) => !BY_ID[id] || itemStatus(BY_ID[id], entries).state === 'done').length;
  return { planned: ids.length, done, complete: done === ids.length };
}
