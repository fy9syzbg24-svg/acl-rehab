...
function advance() {
  ...
  const after = A.endToneLeft() + (bell ? A.BELL_RUNG : 0) + 0.05;
  if (run.phase === 'running') {        // a rest that starts by itself
    ...
    sayLineAt(tapLine(s), `${segName(s)}, tap to start`, after);
    refill();
  }
...
function effects({ carry = false } = {}) {
  ...
  if (mode === 'sound') A.scheduleCues(left, { halfwayAt: saidHalf ? null : halfIn });
  else if (mode === 'buzz') {
