...
export function gapWords(g, legs, unit = g.unit || '') {
  ...
  const gap = (v) => r1(Math.abs(g.value - v));
  const u = unit ? ` ${unit}` : '';
  ...
  if (have.length === 1) return `${have[0].leg === 'B' ? '' : `${cap(LEG[have[0].leg])} `}${gap(have[0].value)}${u} ${dir}`;
  if (short.length === have.length) {
    return `Both legs short of the goal: ${have.map((x) => `${LEG[x.leg]} ${gap(x.value)}${u}`).join(', ')} ${dir}`;
  }
  const met = have.filter((x) => meets(g, x.value));
  return `${met.map((x) => cap(LEG[x.leg])).join(' and ')} at the goal, ${short.map((x) => `${LEG[x.leg]} ${gap(x.value)}${u}`).join(', ')} ${dir}`;
}
...
/** "Goal 95 lb by 23 Oct" or "Discharge goal over 100 lb". */
