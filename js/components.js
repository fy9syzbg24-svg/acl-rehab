...
import { esc, el, round, fromIso, fmtDate, fmtDateShort, addDays, todayIso } from './util.js';
...
import { state, customExercise, update } from './store.js';
import { stepsFor, stepHtml } from './progressions.js';
import { hapTaps, hapInput } from './feedback.js';

/**
 * The primary action: a turquoise rectangle, a play symbol in a circle, the
 * label, an arrow. With `haptic` (Today's Start and Resume only, B6-1) it is a
 * label around a hidden switch where a finger can feel that (hapTaps), the same
 * look in the same place; `disabled` is written the right way for either.
 */
export function goButton({ attrs = '', label, cls = '', haptic = false, disabled = false }) {
  const inner = `
  ...
  `;
  if (haptic && hapTaps()) {
    return `<label class="btn go primary ${cls}" role="button" tabindex="0" ${attrs}${disabled ? ' aria-disabled="true"' : ''}>${hapInput(disabled)}${inner}</label>`;
  }
  return `<button class="btn go primary ${cls}" ${attrs}${disabled ? ' disabled' : ''}>${inner}</button>`;
}
...
export function pictureFor(exId) {
  if (IMG_BY_EX[exId]) return IMG_BY_EX[exId];
  // An exercise from a clinic note can carry a picture from the shell's
  // img/ex/ folder (his pictures, 2026-09-18). Only that folder is honoured:
  // the path comes from synced data, so nothing else may be loaded from it.
  const c = customExercise(exId);
  const ok = (u) => typeof u === 'string' && /^img\/ex\/[a-z0-9-]+\.(jpg|png)$/.test(u);
  if (c && ok(c.img)) return { img: c.img, thumb: ok(c.thumb) ? c.thumb : c.img, title: c.name, pid: exId };
  return null;
}

/** A drawn pictogram, for exercises that never came with a photograph. */
export function iconTile(exId, size = 46) {
  const ex = EXERCISE_BY_ID[exId] || customExercise(exId);
  const cat = ex?.cat || 'strength';
  const glyph = ICONS[iconNameFor(exId, cat)];
  const colour = ex?.tint || CATEGORIES[cat]?.color || 'var(--ink-3)';
...
export function thumb(exId, size = 46) {
  // The custom workout's own picture (his pick, 2026-09-18).
  if (exId === 'custom_workout') {
    // On his blue grey tile (#D3DBE2, his pick), with a baked in drop shadow,
    // and a one pixel edge drawn on the figure itself so it lifts off the tile.
    return `<span class="thumb cw-tile" style="width:${Math.round(size * 16 / 9)}px;height:${size}px;padding:${Math.round(size * 0.05)}px 0">
      <img src="img/custom-workout-4-thumb.png" alt="" decoding="async"></span>`;
  }
...
export function openPicture(pid) {
  const p = REHAB_PROGRAM.find((x) => x.id === pid);
  if (!p) {
    // A clinic exercise with his picture (2026-09-18): the picture and its name.
    const c = pid && !EXERCISE_BY_ID[pid] ? pictureFor(pid) : null;
    if (!c) return;
    openModal({
      title: c.title,
      wide: true,
      body: `<img src="${esc(c.img)}" alt="${esc(c.title)}" style="width:100%;border-radius:var(--radius-sm);display:block">`,
    });
    return;
  }
  // The video, when there is one, plays over the pictures on a tap (videos.js).
  // An item whose only picture is the clinic's own (no public image) shows the
  // private poster in the same place.
  const vid = videoFor(p.id);
  ...
  const pic = p.img
    ...
    : vid ? `<img data-vidposter alt="${esc(p.title)}" class="vid-poster">` : '';
  const back = openModal({
    title: `${p.n}. ${p.title}`,
    wide: true,
    body: `
      ${pic ? `<div class="pic-box">${pic}</div>` : ''}
      ...
      ${steps.length ? `<div style="margin-top:.7rem"><div class="section-title">Progressions</div>
        <ol class="steps">${steps.map((s) => `<li>${stepHtml(s)}</li>`).join('')}</ol></div>` : ''}`,
  });
  loadVideos().then(() => {
    const box = back.querySelector('.pic-box');
    const img = box?.querySelector('img');
    if (!box || !img) return;
    if (img.hasAttribute('data-vidposter')) fillPoster(img, p.id);
    mountVideo(box, img, p.id);
  });
}
...
export function loadBars(series, height = 30) {
  ...
  return `<div class="bars" style="height:${height}px">${series.map((s, i) => {
    ...
    return `<i style="height:${h}px" class="${top ? 'now' : ''}"
      title="${esc(fmtDateShort(s.date))}: ${round(s.load, 2)} ${esc(s.unit)}"></i>`;
  }).join('')}</div>`;
}
...
export function toast(html, kind = 'good', { key = null, ms = 4200 } = {}) {
  ...
  if (had) {
    ...
    // A handle here too (2026-09-23 audit): actionToast binds the new button
    // through it, and without one a second tick's Undo was dead.
    return { dismiss: () => dismiss(k, true), el: had.el, key: k };
  }
  ...
  return { dismiss: () => dismiss(k, true), el: t, key: k };
}

/**
 * A toast with one action on it (item 10, 2026-09-15). His words: "I want to be
 * able to undo logs and change and see what I've done so far."
 *
 * Undo was always possible by tapping the tick again, but nothing ever said so.
 * This puts the way back on screen at the moment it is useful, and takes it away
 * when it stops being true. The action closes the toast itself, so the offer can
 * never outlive the thing it would undo.
 */
export function actionToast(html, label, onAct, { kind = 'good', key = null, ms = 6000 } = {}) {
  const k = key || `act|${label}|${html}`;
  const handle = toast(`<span class="toast-msg">${html}</span><button type="button" class="toast-act">${label}</button>`, kind, { key: k, ms });
  const t = (handle && handle.el) || null;
  t?.querySelector('.toast-act')?.addEventListener('click', () => {
    handle.dismiss();
    try { onAct(); } catch { /* the caller's problem, never the toast's */ }
  });
  return handle;
}
...
// -------------------------------------------------------- exercise picker --
/**
 * A logged row's name. Rows done with blood flow restriction carry `bfr` and
 * read "BFR" first (his ask, 2026-09-18: "those exercises are very difficult
 * and should be easily identifiable in the app").
 */
export const rowName = (e, name) => (e?.bfr ? `BFR ${name}` : name);
...
export function exerciseById(id) {
  return EXERCISE_BY_ID[id] || customExercise(id);
}
...
export function openExercisePicker({ monthN, onPick }) {
  const render = (q, showAll) => {
    const list = allExercises().filter((x) => {
      if (x.timer) return false;   // the custom workout starts from its own card
      ...
      .map(([cat, xs]) => `
        ...
        ${xs.map((x) => `
          <div class="picker-item" data-ex="${esc(x.id)}">
            ${thumb(x.id, 34)}
            <span class="picker-name">${esc(x.name)}${x.clinic ? ' <span class="pill">clinic</span>' : ''}</span>
            <span class="picker-sides" role="group" aria-label="Add ${esc(x.name)} for">
              <button class="btn sm" data-side="L" data-ex="${esc(x.id)}" aria-label="Left leg">L</button>
              <button class="btn sm" data-side="R" data-ex="${esc(x.id)}" aria-label="Right leg">R</button>
  ...
  openModal({
    ...
    onMount(root) {
      ...
      root.querySelector('#pk-add').addEventListener('click', () => {
        ...
        // Through update(), or the record is unstamped and the receiving device
        // dates it, not this one (audit F33, 2026-09-19).
        update(() => { state.data.customExercises.push(ex); });
        closeModal();
        onPick(ex, 'B');
      });
    },
 ...
 */
/**
 * `today` is which date the "today" button returns to. It defaults to the real
 * calendar date; only the supplement checklist overrides it, because its day
 * runs to 5am (see currentDayIso in util.js).
 */
...
export function bindDatePill(root, iso, onChange, { today = todayIso() } = {}) {
  root.querySelectorAll('[data-nav]').forEach((b) => b.addEventListener('click', () => {
    const d = b.dataset.nav;
    if (d === 'today') return onChange(today);
