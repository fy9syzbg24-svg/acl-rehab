...
// History keeps its own week (ctx.histWeek, 2026-09-22): it used to follow
// Today's browsed date, and its arrows moved Today's date seven days at a time.
const histAnchor = (ctx) => ctx.histWeek || todayIso();
...
export function renderWeekPanel(ctx) {
  ...
  const rows = targets.map((t) => {
    ...
    return `<div class="targetrow">
      <span class="small tlabel">${esc(t.label)}
        ${t.src === 'plan' ? '<span class="tsrc">From the plan</span>' : ''}
      </span>
      <span class="tcount">
  ...
  return `
    <section class="ov-sec weekhead"><div class="row between weeknav">
        <div class="datenav">
          <button class="daynav-arrow" data-wnav="-7" aria-label="Previous week">${ARROW.left}</button>
          <span class="d">${esc(fmtDate(ws, 'short'))} to ${esc(fmtDate(addDays(ws, 6), 'short'))}</span>
          <button class="daynav-arrow" data-wnav="7" aria-label="Next week">${ARROW.right}</button>
        </div>
        <div class="weekmeta">
          ${month ? `<span class="weekmonth">${esc(month.short || month.name)} · ${esc(month.monthLabel)}</span>` : ''}
          ${ws === weekStart(todayIso()) ? '' : '<button class="btn sm dp-today" data-wnav="now">This week</button>'}
        </div>
    </div></section>
        ...
        <td class="tiny">
          ${target === null
            ...
            : met
              ...
              : daysLeft === 0
                // A past week is a record, not a to-do (2026-09-22 audit).
                ? `<span class="muted">${n} of ${target}</span>`
...
export function bindWeekPanel(root, ctx, rerender) {
  ...
  root.querySelectorAll('[data-wnav]').forEach((b) => b.addEventListener('click', () => {
    ...
    const next = v === 'now' ? null : addDays(anchor, Number(v));
    // The week under the section row pushes in from the side he went (B5-5).
    const to = weekStart(next || todayIso());
    const from = weekStart(anchor);
    ctx.pushDir = to === from ? 0 : to > from ? 1 : -1;
    ctx.histWeek = next && weekStart(next) !== weekStart(todayIso()) ? next : null;
    rerender();
  }));
  ...
  root.querySelectorAll('[data-target]').forEach((inp) => inp.addEventListener('change', () => {
    update((d) => {
      ...
      // An emptied box goes back to the plan's number, never a stored 0
      // (which read "0/0 Goal met").
      if (inp.value === '') delete d.settings.weeklyOverrides[inp.dataset.target];
      else d.settings.weeklyOverrides[inp.dataset.target] = Number(inp.value);
    });
    rerender();
  }));
}
