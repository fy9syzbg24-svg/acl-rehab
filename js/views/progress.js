...
import { renderOverview, bindOverview, weekBar, bindWeekBar, insightsRow } from './overview.js';
...
import { railShowing } from '../railpages.js';
...
// 2026-09-19: a fifth section after Clinical when his data holds a medication
// (medlevel.js); its name comes from the data, so the public shell names none.
// 2026-09-20: and a sixth for the ring, only once a ring layer has actually
// loaded on this device. Both appear from data, so neither is a dead tab.
// 2026-09-20, his ask: where the two have a place of their own beside the tabs
// they are not repeated here, so exactly one surface offers each at any moment
// (railpages.js). A phone, and the iPad held upright, still find them here.
const tabs = () => {
  const out = [...TABS];
  if (railShowing()) return out;
  const m = medTabLabel();
  if (m) out.push(['meds', m]);
  const r = recoveryTabLabel();
  if (r) out.push(['recovery', r]);
  return out;
};
const CTABS = [['notes', 'Notes'], ['tests', 'Tests'], ['melbourne', 'Melbourne guide']];

export function renderProgress(ctx) {
  let tab = ctx.gtab || 'overview';
  if (tab === 'meds' && (railShowing() || !medTabLabel())) tab = 'overview';
  if (tab === 'recovery' && (railShowing() || !recoveryTabLabel())) tab = 'overview';
  ...
  return `<div class="stack progress">
    ...
    ${tab === 'recovery' ? renderRecovery(ctx) : ''}
  </div>`;
}
...
export function bindProgress(root, ctx, rerender) {
  let tab = ctx.gtab || 'overview';
  if ((tab === 'meds' || tab === 'recovery') && railShowing()) tab = 'overview';
  const ctab = ctx.ctab || 'notes';
  ...
  if (tab === 'meds' && !railShowing() && medTabLabel()) bindMedLevel(root, ctx, rerender);
  if (tab === 'recovery' && !railShowing() && recoveryTabLabel()) bindRecovery(root, ctx, rerender);
  ...
  <div class="stack">
    ...
    <div class="grid2">
      <section class="ov-sec">
        ...
        <div>
          ${(() => {
            // The legend names only the lines drawn (it always said Left and Right).
            const lines = [
            ...
            if (!lines.length) return '<div class="empty">Log a check-in or two and this fills in.</div>';
            return `${lineChart(lines, { height: 140 })}
          <div class="legend" style="margin-top:.4rem">${lines.map((x) => `<span><i style="background:var(--${x.color})"></i>${x.label}</span>`).join('')}</div>`;
          })()}
        </div>
      </section>
    ...
    rows.push(`<div class="tline-day">
      ...
      ${check ? `<div class="tline-row two">
        <span class="tline-main"><b>Knee check-in</b><span class="tline-sub">${[
          (num(c.painL) != null || num(c.painR) != null) ? `pain ${[num(c.painL) != null ? `L ${c.painL}` : '', num(c.painR) != null ? `R ${c.painR}` : ''].filter(Boolean).join(' · ')}` : '',
...
function timeline(today) {
  ...
  const rows = [
    ...
    { label: 'Left knee reaches 9 months', date: addMonths(L, 9), tone: 'left', note: 'Melbourne guide: research suggests a minimum of 9 months before return to sport, guided by your surgeon.' },
    { label: 'Right knee reaches 9 months', date: addMonths(R, 9), tone: 'right', note: 'Same 9-month marker for the right knee.' },
...
function gapNote() {
  ...
  return `<div class="tiny muted" style="margin-top:.6rem">
    ...
    sits <strong>${Math.abs(gap)} day${Math.abs(gap) === 1 ? '' : 's'} ${gap < 0 ? 'before' : 'after'}</strong> the right knee reaches
