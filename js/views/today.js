...
import { itemStatus, isDone, sidesFor, setLogged, newEntriesFor as makeEntries, runsFor, inferCollagen} from '../logging.js';
import { startExercise, startWorkout, resumePlayer, draftInfo, workoutQueue, readyAfter, fmtTime12, logDateFor } from '../player/player.js';
 ...
 * taken, so mark it thirty minutes earlier (item 19, his words: "if I log
...
function inferCollagenFromFirst(iso, at) {
  ...
  return inferCollagen(state.data, ensureDay(iso), at);
}
...
function dayHead(iso, plannedAll, extras, entries, ctx) {
  ...
  const planned = oneTendon(state.data, iso, plannedAll);
  ...
  const extraDone = doneExtras(entries);
  const allDone = planned.length > 0 && doneP.length >= planned.length;
  const ring = dayRing(planned.concat(extraDone.items), entries.concat(extraDone.rows),
    { size: 118, stroke: 10, center: 'count', label: extraDone.items.length || allDone ? 'done' : 'planned',
 ...
 */
function doneExtras(entries) {
  const items = [];
  const rows = [];
  const seen = new Set();
  for (const e of entries) {
    if (e.pid || !e.logged || !e.ex) continue;
    const key = e.ex === CUSTOM_EX ? `run:${e.id}` : `ex:${e.ex}`;
...
function statusLine(iso, planned, entries, ctx = {}) {
  ...
  if (d && d.phase !== 'done') {
    ...
    if (!next) {
      ...
      // first: it is thirty minutes after his collagen and the day is built
      // around it.
      return '';
    }
  } else {
...
function workoutButton(iso) {
  ...
  const plan = plannedItems(iso);
  const empty = iso === todayIso() && !resume && plan.length > 0 && left === plan.length;
  return goButton({
    attrs: `data-act="${resume ? 'resume' : 'start'}" ${resume || left ? '' : 'disabled'} aria-label="${resume ? `Resume ${esc(d.title || 'workout')}${esc(other)}` : future ? `Start ${esc(fmtDate(iso, 'dow'))}'s workout, logged today` : 'Start the workout'}"`,
    ...
    <div class="crow-head">
      ...
      <button class="crow-main" data-rowclick="${esc(item.id)}" aria-expanded="${open}" aria-controls="${bodyId}">
        <span class="crow-text">
          ...
          <span class="crow-sub">${confirmed.length ? entryChips(confirmed) + statusNote(status) : prescriptionLine(withGoals(state.data, item, iso), band)}</span>
...
function tickItem(d, item, on, iso = null) {
  ...
  // Ticking the tendon loading marks the collagen thirty minutes earlier when
  ...
  if (!mine.length) { d.entries.push(...newEntriesFor(item, exerciseById(item.ex), true)); return inferred; }
    ...
    onMount(root) {
      root.querySelectorAll('[data-m]').forEach((b) => b.addEventListener('click', () => {
        ...
        if (act === 'tickall') {
          ...
          for (const g of guessed) weakenDayPath(iso, `supps.${g}`);
        }
        if (act === 'repeat') {
          const prev = Object.keys(state.data.days)
...
function drawRingSegment(page, iso, pid) {
  ...
  const planned = plannedItems(iso);
  const i = planned.findIndex((p) => p.id === pid);
  if (i < 0) return;
  const seg = page.querySelectorAll('.dayring2 .dr-seg.on, .dr-seg.on')[i];
...
export function bindToday(root, ctx, rerender) {
  ...
  root.querySelectorAll('[data-ptoggle]').forEach((cb) => cb.addEventListener('change', () => {
    ...
    if (!item) return;
    haptic('tick');   // item 7, only when he has switched it on
    let inferred = null;
    update(() => {
      const d = ensureDay(iso);
      inferred = tickItem(d, item, cb.checked, iso);
    });
    if (inferred) weakenDayPath(iso, `supps.${inferred}`);
    if (cb.checked) {
      testToast(recordAsTests(iso, ensureDay(iso).entries.filter((e) => e.pid === pid)));
      ...
      actionToast(`Logged ${esc(item.title || 'exercise')}`, 'Undo', () => {
        update(() => { tickItem(ensureDay(iso), item, false); });
        rerender();
      }, { key: 'tick-undo' });
    }
      ...
      onPick(ex, side) {
        ...
        if (item) {
          update(() => {
            const d = ensureDay(iso);
            const mine = d.entries.filter((e) => e.pid === item.id);
            if (mine.length) { for (const e of mine) e.added = true; return; }
            const rows = newEntriesFor(item, exerciseById(item.ex), false)
