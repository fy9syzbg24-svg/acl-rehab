...
import { soundCheck, sessionKind, cueVolume, setCueVolume, unlockAudio, loadVoice, audioAvailable, stereoOn, setStereo, stereoAvailable,
  voiceVolume, setVoiceVolume, preferVoice, whenVoiceReady, playVoice, coachDemo, COACH_HITS, COACH_LINES } from '../player/audio.js';
...
export function renderSettings(ctx = {}) {
  ...
  const sum = {
    surgeries: [s.surgeryLeft && `L ${fmtDateShort(s.surgeryLeft)}`, s.surgeryRight && `R ${fmtDateShort(s.surgeryRight)}`].filter(Boolean).join(' · ') || 'not set',
    ...
    oura: ringNewest ? `Newest night ${fmtDateShort(ringNewest)}` : ringSettled() ? 'Nothing here yet' : 'Loading',
  ...
  <div class="stack settings-page">
    ...
    ${group('sound', 'Workout sound')}
      <div class="card-body">
        <div class="sc-tiles n4 snd-facts">
          <div class="sc-tile" style="--i:0"><small>Your music</small><b>Never paused</b><span>Cues duck under it, then it comes straight back</span></div>
          <div class="sc-tile" style="--i:1"><small>App in the background</small><b>No cues</b><span>Measured on your phone</span></div>
          <div class="sc-tile" style="--i:2"><small>Phone on Silent</small><b>No sound cues</b><span>Measured on your phone</span></div>
          <div class="sc-tile" style="--i:3"><small>Your own songs</small><b>Cues keep going</b><span>Other music pauses, even with the screen off</span></div>
        </div>
        ...
        <label class="fld" style="max-width:280px;margin:0 0 .6rem">Voice volume
          <input type="range" min="0.2" max="1" step="0.05" data-voicevol value="${voiceVolume()}">
        </label>
        <div class="row"><button class="btn" data-soundtest>Test sound</button><button class="btn" data-coachdemo><span class="cd-dial" aria-hidden="true">${DIAL_ICON}</span><span>Hear the coach</span></button><span class="tiny muted" data-soundtest-out></span></div>
        <div class="tiny muted" style="margin-top:.4rem">${esc(audioModeLine())}</div>

        <label class="check-row" style="margin-top:.9rem"><input type="checkbox" ${sw()} data-speak ${speakOn() ? 'checked' : ''} ${speechAvailable() || audioAvailable() ? '' : 'disabled'}>
          <span><b>Coach voice</b><br>
          <span class="muted">A recorded voice calls each step: "Get ready", "Left leg", "Last set", "Ten seconds", "Next up".</span></span></label>

        ${stereoAvailable() ? `<label class="check-row"><input type="checkbox" ${sw()} data-stereo ${stereoOn() ? 'checked' : ''}>
          <span><b>Stereo sides</b><br>
          <span class="muted">Left from the left ear, right from the right, with headphones</span></span></label>` : ''}

        ${hapticsAvailable() ? `<label class="check-row"><input type="checkbox" ${sw()} data-haptics ${hapticsOn() ? 'checked' : ''}>
          <span><b>Touch feedback</b><br>
          <span class="muted">A short tap you can feel on a tick and a step change.</span></span></label>` : ''}
      </div>
    </details>

    ${group('reminders', 'Reminders')}
      <div class="card-body">
        <label class="check-row"><input type="checkbox" ${sw()} data-reminders ${remindersOn() ? 'checked' : ''} ${pushSupported() ? '' : 'disabled'}>
          <span><b>Remind me on this device</b><br>
          <span class="muted">The tendon loading an hour after your collagen, the six hour line after the first session, and a twice a day dose. Tapping one opens the app, and they clear themselves when you do.</span></span></label>
        ...
        <div ${remindersOn() ? '' : 'hidden'} data-remind-what>
          ...
          <label class="check-row" style="margin-top:.4rem"><input type="checkbox" ${sw()} data-quiet ${remindPrefs().quietOn ? 'checked' : ''}>
    ...
    ${group('badge', 'Home Screen badge')}
      <div class="card-body">
        <label class="check-row"><input type="checkbox" ${sw()} data-badge ${badgeOn() ? 'checked' : ''} ${badgeAvailable() ? '' : 'disabled'}>
        ...
        <div ${badgeOn() ? '' : 'hidden'} data-badge-what>
          ...
          ${BADGE_PARTS.map(([k, label]) => `<label class="check-row"><input type="checkbox" ${sw()} data-badgepart="${k}" ${badgeParts().includes(k) ? 'checked' : ''}><span>${esc(label)}</span></label>`).join('')}
        </div>
      </div>
    </details>
    ...
    ${group('oura', 'Oura ring')}
      <div class="card-body">
        <div class="callout small" style="margin-bottom:.8rem">
          ${SERVER_MODE
            ? `Takes the last week from Oura, rebuilds the nightly summary and puts it where
               your phone and iPad read it. It happens on its own every day at noon; this is
               for when you want it now. Read only: nothing is ever written to Oura, and none
               of it goes into your log.`
            : `Only the Mac holds your Oura sign-in, so this asks it to fetch and picks up what
               it has already. If the Mac is on, the new nights land here within about five
               minutes and the page updates itself. Read only, and none of it goes into your log.`}
        </div>
        <div class="row" style="gap:.5rem;flex-wrap:wrap;align-items:center">
          <button class="btn" data-oura-refresh>${SERVER_MODE ? 'Fetch from Oura' : 'Sync ring data'}</button>
          <span class="tiny muted" data-oura-status>${ringNewest
            ? `Newest night ${esc(fmtDateShort(ringNewest))}${ringBuiltAt() ? `, built ${esc(when12(ringBuiltAt()))}` : ''}`
            : ringSettled() ? 'No ring data on this device yet' : ''}</span>
        </div>
        ${ringSource() === 'cache' ? `<div class="tiny muted" style="margin-top:.35rem">
          Showing this device's last copy, so it is working offline.
        </div>` : ''}
      </div>
    </details>
...
export function bindSettings(root, ctx, rerender) {
  ...
  root.querySelector('[data-coachdemo]')?.addEventListener('click', async (e) => {
    ...
    if (!unlockAudio()) { if (out) out.textContent = 'Sound is not available in this browser.'; return; }
