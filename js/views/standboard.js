...
import { esc, round, fmtDate, num } from '../util.js';
...
export function sinceLast(m, leg) {
  ...
  if (!prev) return null;
  return { delta: round(last.value - prev.value, 2), from: prev.date, to: last.date };
}
...
function perKgWords(m, legs, p) {
  ...
  return `Per kg of bodyweight: left ${f(L)}, right ${f(R)} N/kg`;
}
...
function pctileWords(region) {
  ...
  for (const id of region.tests) {
    ...
    bits.push(got.length === 1 && !got[0].leg
      ? `${name}: ${ORD(got[0].r.pctile)} percentile`
      : `${name}: ${got.map((x) => `${x.leg === 'L' ? 'left' : 'right'} ${ORD(x.r.pctile)}`).join(', ')} percentile`);
    for (const x of got) date = !date || x.r.date > date ? x.r.date : date;
  }
  return bits.length ? `${bits.join(' · ')}${date ? `, VALD ${fmtDate(date, 'short')}` : ''}` : '';
}

/** The latest manual muscle test grades for the region. */
function mmtWords(region) {
  ...
  for (const id of REGION_MMT[region.id] || []) {
    ...
    if (!L && !R) continue;
    const d = [L?.date, R?.date].filter(Boolean).sort().pop();
    if (date && d < date) continue;
    date = d;
    bits.push(`${m.label.toLowerCase()} ${L ? `left ${L.value}` : ''}${L && R ? ', ' : ''}${R ? `right ${R.value}` : ''}`);
  }
  return bits.length ? `Manual muscle test: ${bits.join(' · ')}, ${fmtDate(date, 'short')}` : '';
}
...
export function regionCard(region) {
  ...
  const head = withData.find((x) => x.m.perLeg) || withData[0];
  ...
  if (region.graphic === 'arcs') {
    ...
    graphic = m.perLeg ? bulletBars(m, legs, rails) : bulletBars(m, { L: legs.B, R: null }, rails);
  }
  ...
  return `<article class="rcard" data-region="${esc(region.id)}">
    ...
    <div class="rcard-words">
      ...
      ${extra.map((t) => `<span class="rextra">${esc(t)}</span>`).join('')}
...
export function regionGap(region) {
  ...
  const x = head.find((h) => h.m.perLeg) || head[0];
  if (!x) return null;
...
export function renderBoard() {
  ...
  <section class="board" aria-label="Where you stand, by region">${list.map((x) => x.html).join('')}</section>`;
}
