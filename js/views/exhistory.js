...
import { esc, num, round, fmtDateShort, toKg, fromKg } from '../util.js';
 ...
 */
/**
 * A session's rows as words. Left and right that match read once, "4 × 5 each
 * side" (his call, 2026-09-18: sets and reps are the same on both legs); only
 * sides that differ are written out per leg.
 */
export function sidesText(rows, item, unit) {
  const texts = rows.map((e) => rowText(e, unit));
  const L = rows.findIndex((e) => e.side === 'L');
  const R = rows.findIndex((e) => e.side === 'R');
  if (rows.length === 2 && L >= 0 && R >= 0 && texts[L] === texts[R]) return `${esc(texts[L])} each side`;
  return rows.map((e, i) => `${sideLabel(e, item)}${esc(texts[i])}`).join(' · ');
}
...
export function summaryLine(doc, item, iso) {
  ...
  if (todayDone.length) return { label: 'This session', html: sidesText(todayDone, item, unit) };
  const last = historyFor(doc, item, iso)[0];
  if (last) return { label: `Last session · ${fmtDateShort(last.date)}`, html: sidesText(last.rows, item, unit) };
  return { label: 'Last session', html: '<span class="muted">Nothing confirmed yet</span>' };
}

/**
 * `compact` (Today's open row, 2026-09-18): the row already shows this or last
 * session just above, so the box keeps only the target, the best load and the
 * recent sessions, on as few lines as it can.
 */
export function renderHistory(doc, item, iso, { compact = false } = {}) {
  const unit = doc.settings?.weightUnit || 'kg';
  ...
  if (compact) {
    return `<div class="exhist compact">
      ...
      ${bestSides.length ? `<div class="exh-line"><span class="exh-k">Best load</span><span class="exh-v">${bestSides
        .map(([s, b]) => `${s === 'B' ? '' : `<b class="sidetag ${s}">${s}</b> `}${esc(String(round(fromKg(b.kg, unit), 1)))} ${esc(unit)}`).join(' · ')}</span></div>` : ''}
      ${recent.length > 1 || notes.length ? `<details class="exh-more" data-exh="${esc(item.id)}" ${openRecent.has(item.id) ? 'open' : ''}>
        ...
        <ul class="exh-list">${recent.map(({ date, rows }) => `<li><span class="mono">${esc(fmtDateShort(date))}</span> ${sidesText(rows, item, unit)}</li>`).join('')}</ul>
        ${notes.length ? `<ul class="exh-list notes">${notes.map((n) => `<li><span class="mono">${esc(fmtDateShort(n.date))}</span> ${esc(n.text)}</li>`).join('')}</ul>` : ''}
      </details>` : ''}
    </div>`;
  }
  return `<div class="exhist">
    ...
    <div class="exh-line"><span class="exh-k">Last session</span><span class="exh-v">${last
      ? `${esc(fmtDateShort(last.date))} · ${last.rows.map((e) => `${sideLabel(e, item)}${esc(rowText(e, unit))}`).join(' · ')}${last.rows.some((e) => e.via === 'physiapp') ? '<span class="srcnote">PhysiApp</span>' : ''}`
      : '<span class="muted">nothing confirmed yet</span>'}</span></div>
    ${bestSides.length ? `<div class="exh-line"><span class="exh-k">Best load</span><span class="exh-v">${bestSides
      .map(([s, b]) => `${s === 'B' ? '' : `<b class="sidetag ${s}">${s}</b> `}${esc(String(round(fromKg(b.kg, unit), 1)))} ${esc(unit)} <span class="tiny muted">${esc(fmtDateShort(b.date))}</span>`)
      .join(' · ')}</span></div>` : ''}
    ${recent.length > 1 || notes.length ? `<details class="exh-more" data-exh="${esc(item.id)}" ${openRecent.has(item.id) ? 'open' : ''}>
      <summary>Recent sessions${notes.length ? ' and notes' : ''}</summary>
      <ul class="exh-list">${recent.map(({ date, rows }) => `<li><span class="mono">${esc(fmtDateShort(date))}</span> ${rows.map((e) => `${sideLabel(e, item)}${esc(rowText(e, unit))}`).join(' · ')}</li>`).join('')}</ul>
      ${notes.length ? `<ul class="exh-list notes">${notes.map((n) => `<li><span class="mono">${esc(fmtDateShort(n.date))}</span> ${esc(n.text)}${n.via === 'physiapp' ? ' <span class="srcnote">PhysiApp</span>' : ''}</li>`).join('')}</ul>` : ''}
    </details>` : ''}
  </div>`;
}
