...
export function summaryLine(doc, item, iso) {
  ...
  if (last) return { label: `Last session · ${fmtDateNum(last.date)}`, html: sidesText(last.rows, item, unit) };
  ...
  if (compact) {
    return `<div class="exhist compact">
      ...
      ${recent.length > 1 || notes.length ? `<details class="exh-more" data-exh="${esc(item.id)}" ${openRecent.has(item.id) ? 'open' : ''}>
        ...
        <ul class="exh-list">${recent.map(({ date, rows }) => `<li><span class="mono">${esc(fmtDateNum(date))}</span> ${sidesText(rows, item, unit)}</li>`).join('')}</ul>
        ${notes.length ? `<ul class="exh-list notes">${notes.map((n) => `<li><span class="mono">${esc(fmtDateNum(n.date))}</span> ${esc(n.text)}</li>`).join('')}</ul>` : ''}
      </details>` : ''}
    </div>`;
  }
