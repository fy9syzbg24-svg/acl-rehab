...
import { hapTaps } from '../feedback.js';
 ...
 *
 * The row is already a label around its checkbox, so where a finger can feel
 * a switch (B6-1, hapTaps) that checkbox simply gets the `switch` attribute:
 * it stays hidden, the tick is still drawn by .supptick, and his tap on the
 * row toggles it, which iOS answers with its own tap. Nothing else changes.
 */
  ...
  return `
    <div class="supprow ${on ? 'on' : ''} ${ctx.suppPop === s.id ? 'pop' : ''}" data-row="${esc(s.id)}">
      ...
      ${edit
        ...
        : `<label class="supptime ${on ? '' : 'inactive'} ${t ? 'set' : ''} ${next ? 'hint' : ''} ${next && next.getTime() <= Date.now() ? 'due' : ''}" data-supptime-wrap>
            <span${next ? ` aria-label="Next dose about ${esc(time12(next))}${next.getTime() <= Date.now() ? ', you can take it now' : ''}"` : ''}>${t ? esc(time12(t)) : next ? `Next ${esc(time12(next))}` : 'Set time'}</span>
    ...
    <header class="pagehead">
      ...
      <div class="daynav supps-nav">
        ...
        <span class="daynav-ctl">
          ${iso === cur ? '' : '<button class="btn sm daynav-today" data-nav="today">Today</button>'}
      ...
      <div class="supps-bar">
        <span class="supps-count">${score ? `<span data-roll>${score.taken} of ${score.total}</span> taken` : 'Nothing on the list'}</span>
...
export function bindSupplements(root, ctx, rerender) {
  ...
  // header does what the arrows do (swipe.js).
  const toDay = (next) => {
    if (!next) return;
  ...
  bindDatePill(root, iso, toDay, { today: currentDayIso() });
