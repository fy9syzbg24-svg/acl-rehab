  ...
  for (const [date, day] of Object.entries(state.data.days)) {
    for (const e of day.entries || []) {
      ...
      for (const s of sides) {
        const key = `${e.ex}|${s}`;
        const cur = rec[key];
        const cand = { ex: e.ex, side: s, load, unit: e.loadUnit || 'kg', sets: e.sets, reps: e.reps, date, seeded: e.seeded, clinic: e.clinic, both: e.side === 'B' };
...
function historyView(ctx) {
  ...
  const rows = allMeasurements().slice().sort((a, b) => (a.date < b.date ? 1 : -1));
  return `
