...
/**
 * Lock Screen and Control Center controls for his songs (item 4, 2026-09-15).
 *
 * The Media Session API is what puts a title and working play/pause on the Lock
 * Screen. It is metadata and controls only: it does NOT keep the page running in
 * the background and is not a permission for anything, so nothing else in the
 * player depends on it.
 *
 * Play and pause are wired to the same functions the in-app buttons call, so the
 * two can never disagree, and Next moves the queue exactly as Skip does. Seeking
 * is deliberately not offered: the workout clock is not the song's clock, and a
 * scrub bar that looked like it moved the workout would be a lie.
 */
function mediaControls() {
  const ms = typeof navigator !== 'undefined' && navigator.mediaSession;
  if (!ms) return;
  try {
    ms.setActionHandler('play', () => playSong(true));
    ms.setActionHandler('pause', () => pauseSong());
    // Next on the Lock Screen really moves on (it only read the next track and
    // replayed the same one, 2026-09-22 audit).
    ms.setActionHandler('nexttrack', () => { skipSong(); });
    ms.setActionHandler('previoustrack', null);
    ms.setActionHandler('seekto', null);
    ms.setActionHandler('seekbackward', null);
    ms.setActionHandler('seekforward', null);
  } catch { /* an unsupported action throws on its own, the rest still bind */ }
}

/** Title and state on the Lock Screen. Safe to call on every status change. */
export function paintMediaSession() {
  const ms = typeof navigator !== 'undefined' && navigator.mediaSession;
  if (!ms) return;
  try {
    ...
    if (song && window.MediaMetadata) {
      ms.metadata = new window.MediaMetadata({
        title: song.title || song.name || 'Workout track',
        artist: song.artist || 'Rehab tracker',
      });
    }
