 ...
 * centre, never catching a tap. Everything is drawn in px around 0,0.
 */
function stage(host, cx, cy, span, cls) {
  const svg = el('svg', {
...
export function orbitSeal({ title = '', ringEl = null } = {}) {
  return new Promise((resolve) => {
    ...
    const finish = () => {
      ...
      layer.animate([{ opacity: 1 }, { opacity: 0 }], { duration: 220, easing: EASE_IN_OUT })
        .finished.catch(() => {}).then(() => { layer.remove(); resolve(); });
    };
