...
import { haptic, buzz, say, hush, scheduleBuzzes, cancelBuzzes } from '../feedback.js';
import { goldEcho, wakeLabel, orbitSeal, constellation, RING_VIEW, RING_R, RING_STROKE } from './celebrate.js';
import { state, update, ensureDay, getDay, lastEntry, lastCardioMinutes, flushSave, weakenDayPath } from '../store.js';
...
import { itemStatus, saveRun, rowsFingerprint, runsFor, inferCollagen } from '../logging.js';
...
export function startExercise(ctx, pid, planIso = ctx.date || todayIso()) {
  ...
  open(ctx, { run: newRun(pid, iso), session: { queue, pos: 0, iso, planIso }, phase: 'run', base: rowsFingerprint(getDay(iso), pid), savedRunIds: [] });
}
...
export function startWorkout(ctx, planIso = ctx.date || todayIso()) {
  ...
  open(ctx, { run: newRun(queue[0], iso), session: { queue, pos: 0, iso, planIso }, phase: 'run',
...
function syncEffects() {
  ...
  if (cuesOn() && rem != null) {
    ...
    // and it carries on.
    if (st?.open) {
      if (rem > 0.05 && A.silentMode() !== 'buzz') A.scheduleSwitch(rem);
      if (rem > 0.05) scheduleBuzzes(rem);
    } else {
    if (A.silentMode() !== 'buzz') A.scheduleCues(rem);
    ...
    // Get ready is the one step that leads INTO work, so it is the one that
    // earns a count in (B2). Anywhere else would run two countdowns at once.
    if (st?.kind === 'ready' && cuesHeard()) A.scheduleCountIn(rem);
  }
...
function announceEvents(events) {
  ...
  if (last.type === 'review') { announce('Sets complete. Saving.'); say('Sets complete'); }
...
export function renderPlayer(ctx) {
  ...
  lastDial = { runId: run.runId, working: dialWorking(run) };
  ...
  <div class="player" data-player data-run="${esc(run.runId)}" style="--cat:${cat};--cat-end:${catEnd}">
    ...
    <div class="p-stage"><div class="p-content">
      ...
      <div class="p-dial ${dialWorking(run) ? 'working' : ''}">
      ...
      <div class="p-nextrow"><div class="p-next" data-slot="next">${esc(nextLine(run))}</div></div>
...
function dialMoments(root, run) {
  ...
  lastDial = { runId: run.runId, working };
...
function phaseRow(run) {
  ...
  const word = stepWord(run, kind);
  const dflt = kind === 'rest' && st.restSrc === 'default' ? '<span class="p-default">default</span>' : '';
  const neutral = ['rest', 'ready', 'switch'].includes(kind) ? 'neutral' : '';
...
function ringCenter(run, st, now) {
  ...
  if (back != null) {
    return `<div class="p-num" data-p-clock>${back}</div>
  ...
  if (st.open) {
    const up = fmtClock(E.elapsedMs(run, now) / 1000);
    return `<div class="p-num ${up.length > 4 ? 'long' : ''}" data-p-clock>${up}</div>
      <div class="p-cap">Goal ${st.secs}s</div>`;
  }
...
function paintClock(now) {
  ...
  const txt = fmtClock(stp?.open ? E.elapsedMs(run, now) / 1000 : rem);
...
function cueTool() {
  ...
  const aria = mode === 'sound' ? 'Countdown cues: sound. Tap for buzz.'
...
function startResumeCountdown(ctx) {
  if (!P?.run) return;
  clearResumeCountdown();
  A.unlockAudio();
...
function saveCurrent() {
  ...
  try {
    ...
    update(() => {
      ...
      // The tendon loading marks the collagen thirty minutes earlier when he has
      // not marked it (2026-09-16). This is the path he actually uses; only the
      // tick used to do it.
      if (item?.first) inferred = inferCollagen(state.data, day, new Date()) || null;
    });
    // A guess must never beat a real tick made on another device (rule zero).
    if (inferred) weakenDayPath(run.iso, `supps.${inferred}`);
...
async function finishRun(ctx, { leave = false } = {}) {
  ...
  if (saved === true) {
    ...
    // The workout is written down (B1, 2026-09-15). On every durable save
    // path, and deliberately here rather than where the sets ended, so the
    // sound means "it is saved" and not "you stopped".
    ...
    if (E.summary(run).complete) {
      await orbitSeal({ title: nameOf(item), ringEl: document.querySelector('.player .p-ring') });
    } else if (leave || !P.session || item.first) {
      toast(`<b>Logged</b><br><span>${esc(nameOf(item))}</span>`, 'good', { key: 'player-logged' });
    } else {
  ...
  if (item.first) {
    ...
    if (saved === true) { closePlayer(ctx, currentRerender); return; }
    currentRerender?.();
    return;
  }
  moveOn(P.session);
...
export function bindPlayer(root, ctx, rerender) {
  ...
  if (day && !day.dataset.played) {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => constellation(day.querySelector('.fin-ringhost'), {
  ...
  player?.addEventListener('click', (ev) => {
    ...
    // the run is replaced.
    if (P?.finishing && (TRANSPORT.has(k) || k === 'reps-' || k === 'reps+' || k === 'more')) return;
    ...
    if (k === 'pause') {
      ...
      if (r && r.state === 'paused') {
        ...
        if (P.resumeAt) { clearResumeCountdown(); syncEffects(); refresh(); return; }
    ...
    if (k === 'skip') {
      ...
      act((x, now, wall) => {
        ...
        if (E.WORK.has(E.step(x)?.kind)) {
          if (x.state === 'running') E.pause(x, now, wall);
          if (x.state === 'paused' || x.state === 'interrupted') P.resumeAt = Date.now() + RESUME_SECS * 1000;
        }
        return evs;
      });
      if (P?.resumeAt) startResumeCountdown(ctx);
      return;
    }
    ...
    if (k === 'cues') {
      // Sound -> Buzz -> Off -> Sound. 'through' is a Settings choice and is
      ...
      const next = mode === 'sound' ? 'buzz' : mode === 'buzz' ? 'off' : 'sound';
