...
import { esc, round, fmtDate, fromIso, toKg, fromKg, toIso} from './util.js';
...
// Force tests (newtons or pounds) can be read in N, kgf or lb (2026-09-16), a
// per device choice like the weight unit. The stored value never changes; the
// panel shows the recorded figure beside the converted one (F05 pattern).
const FORCE_KEY = 'rehab.trend.force';
const N_PER = { N: 1, kgf: 9.80665, lb: 4.4482216153 };
export const FORCE_UNITS = ['N', 'kgf', 'lb'];
export const isForce = (m) => m && (m.unit === 'N' || m.unit === 'lb');
export function forceUnit(m) {
  try { const v = localStorage.getItem(FORCE_KEY); if (v && N_PER[v]) return v; } catch { /* per device */ }
  return m ? m.unit : 'N';
}
export function setForceUnit(u) { try { localStorage.setItem(FORCE_KEY, u); } catch { /* per device */ } }
export function convertForce(v, from, to) { return (v * (N_PER[from] || 1)) / (N_PER[to] || 1); }
function unitOf(m, r) {
  if (m.unit === 'weight') return state.data.settings.weightUnit || r?.unit || 'kg';
  if (isForce(m)) return forceUnit(m);
...
function seriesFor(m, range) {
  ...
  const want = m.unit === 'weight' ? (state.data.settings.weightUnit || 'kg') : isForce(m) ? forceUnit(m) : null;
  const norm = (r) => {
    if (!want) return r;
    const from = r.unit || m.unit || want;
    if (from === want) return r;
    const value = isForce(m) ? round(convertForce(r.value, from, want), 3) : round(fromKg(toKg(r.value, from), want), 3);
    return { ...r, value, orig: { value: r.value, unit: from } };
  };
...
function rangeStart(range) {
  ...
  return toIso(d);   // local, not UTC: after 8pm here the window began a day late (audit F32)
}
...
export function renderTrend(ctx, { key = 'overview', width = 640, compact = false, measures = null, refs = [] } = {}) {
  lastOpts[key] = { key, width, compact, measures, refs };
  ...
  // References arrive in the measure's own unit; a force chart may be read in another.
  const fu = isForce(m) ? forceUnit(m) : null;
  const refsIn = (refs || []).map((r) => (fu && fu !== m.unit && typeof r.value === 'number'
    ? { ...r, value: convertForce(r.value, m.unit, fu), label: r.label.replace(/\d[\d.]*$/, String(round(convertForce(r.value, m.unit, fu), 0))) } : r));
  // The goal and the published levels are named here, under the title, and the
  // chart keeps only their number (Fable's idea 3, 2026-09-19): the same words
  // were printed on every rule and crowded the plot.
  const refKey = refsIn.filter((r) => typeof r.value === 'number').map((r) =>
    `<span class="tr-key${r.goal ? ' goal' : ''}"><i></i>${esc(r.label)}</span>`).join('');

  const head = `
    <div class="tr-head">
      <div class="tr-title">
        <label class="tr-pick"><span class="sr-only">Measurement</span>
          <select data-tr-measure ${list.length === 1 ? 'data-one="1" tabindex="-1"' : ''}>${list.map((x) => `<option value="${esc(x.id)}" ${x.id === m.id ? 'selected' : ''}>${esc(x.label)}</option>`).join('')}</select></label>
        <div class="tr-sub">${esc(m.group)}${u ? ` · ${esc(unitName(m, u))}` : ''}${m.lower ? ' · lower is better' : ''}</div>
        ${refKey ? `<div class="tr-keys">${refKey}</div>` : ''}
      </div>
    ...
    <div class="tr-ranges" role="group" aria-label="Range">${RANGES.map(([k, l]) =>
      `<button class="${range === k ? 'on' : ''}" data-tr-range="${k}" aria-pressed="${range === k}">${esc(l)}</button>`).join('')}
      ${isForce(m) ? `<span class="tr-units" role="group" aria-label="Unit">${FORCE_UNITS.map((k) =>
        `<button class="${u === k ? 'on' : ''}" data-tr-unit="${k}" aria-pressed="${u === k}">${k}</button>`).join('')}</span>` : ''}</div>`;
  ...
  // A published reference that can be placed is a neutral rule on the chart
  // (2026-09-16); it widens the scale but is never a target zone.
  const refVals = refsIn.map((r) => r.value).filter((v) => typeof v === 'number');
  const vals = Object.values(series).flat().map((r) => r.value);
  const ticks = niceTicks(Math.min(...vals, ...refVals), Math.max(...vals, ...refVals));
  const y0 = ticks[0];
  ...
  // The label sits just above its rule, or just below it when the rule is at
  // the top of the plot, so nothing is drawn outside the plot.
  // Two rules close together used to print their labels on top of each other
  // (audit F16, 2026-09-19): each label keeps a line's height from the last.
  let lastLabY = -1e9;
  // Top of the plot down, so a label only ever moves away from the rule above
  // it: laid out in list order, a low rule's number could land on a high one.
  const refLines = refsIn.filter((r) => typeof r.value === 'number')
    .slice().sort((a, b) => b.value - a.value).map((r) => {
    const y = py(r.value);
    let ly = y - 5 < M.t + 10 ? y + 12 : y - 5;
    if (Math.abs(ly - lastLabY) < 13) ly = lastLabY + 13;
    lastLabY = ly;
    return `<line class="tr-ref${r.goal ? ' goal' : ''}" x1="${M.l}" x2="${W - M.r}" y1="${y.toFixed(1)}" y2="${y.toFixed(1)}"/>
    <text class="tr-reflab${r.goal ? ' goal' : ''}" x="${W - M.r}" y="${ly.toFixed(1)}" text-anchor="end">${esc((r.label.match(/[\d.]+$/) || [r.label])[0])}</text>`;
  }).join('');
  ...
  const lines = Object.entries(series).map(([leg, pts]) => {
    ...
    const dots = pts.map((r, i) => {
      ...
      // Left points are circles, right points squares (Apple's chart guidance,
      // 2026-09-16): readable without the colour, the colour contract unchanged.
      const cx = px(r.date) + off; const cy = py(r.value);
      return leg === 'R'
        ? `<rect class="tr-pt ${on ? 'sel' : ''}" data-tr-pt="${esc(r.date)}" data-tr-rec="${esc(id)}" x="${(cx - 4).toFixed(1)}" y="${(cy - 4).toFixed(1)}" width="8" height="8"/>`
        : `<circle class="tr-pt ${on ? 'sel' : ''}" data-tr-pt="${esc(r.date)}" data-tr-rec="${esc(id)}" cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="4"/>`;
    }).join('');
...
function unitName(m, u) {
  return { N: 'newtons (N)', kgf: 'kilogram force (kgf)', lb: 'pounds (lb)', s: 'seconds', '°': 'degrees', cm: 'centimetres', mm: 'millimetres', '%': 'per cent',
...
/**
 * The shared cursor (2026-09-16): every chart on the page that holds this
 * date moves to it; a chart without it stays where it is. Pinning also
 * updates each chart's remembered selection.
 */
export function pinAcross(root, ctx, iso, { pin = true, from = null } = {}) {
  for (const [key, c] of Object.entries(charts)) {
    if (!c || !c.dates.includes(iso) || !root.querySelector(`[data-trend="${key}"]`)) continue;
    // The chart that was tapped keeps its own pick (a single record on a date
    // with two results); the others follow the date (2026-09-22 audit).
    if (key === from) continue;
    const st = ctx.trend?.[key];
    if (pin && st) { st.sel = iso; st.rec = null; }
    selectDate(root, key, iso, { pin, rec: null });
  }
}
/** After a hover preview, every chart returns to its own pinned date. */
export function restoreAcross(root, ctx) {
  for (const [key, c] of Object.entries(charts)) {
    const st = ctx.trend?.[key];
    if (!c || !st?.sel || !root.querySelector(`[data-trend="${key}"]`)) continue;
    selectDate(root, key, st.sel, { pin: false, rec: st.rec });
  }
}
...
/**
 * The line draws itself the first time this chart is shown (C3, 2026-09-15).
 *
 * Tied to the same `bound` flag as the bindings, so it happens once per chart
 * and never again: animating it on every repaint would redraw the whole history
 * each time he ticks something, which is noise rather than motion.
 */
function drawLineIn(wrap) {
  if (typeof matchMedia === 'function' && matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  for (const line of wrap.querySelectorAll('.tr-line')) {
    ...
    } catch { /* a path that cannot measure itself simply appears */ }
  }
}
...
export function bindTrend(root, ctx, rerender, { onOpen, onRecord } = {}) {
  root.querySelectorAll('[data-trend]').forEach((wrap) => {
    ...
    wrap.querySelectorAll('[data-tr-unit]').forEach((b) => b.addEventListener('click', () => {
      setForceUnit(b.dataset.trUnit);
      rerender();
    }));
