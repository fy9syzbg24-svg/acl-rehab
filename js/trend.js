...
export function renderTrend(ctx, { key = 'overview', width = 640, compact = false, measures = null, refs = [] } = {}) {
  ...
  const refLines = refsIn.filter((r) => typeof r.value === 'number').map((r) => {
    const y = py(r.value);
    const ly = y - 5 < M.t + 10 ? y + 12 : y - 5;
    return `<line class="tr-ref${r.goal ? ' goal' : ''}" x1="${M.l}" x2="${W - M.r}" y1="${y.toFixed(1)}" y2="${y.toFixed(1)}"/>
    <text class="tr-reflab${r.goal ? ' goal' : ''}" x="${W - M.r}" y="${ly.toFixed(1)}" text-anchor="end">${esc(r.label)}</text>`;
  }).join('');
 ...
 */
export function pinAcross(root, ctx, iso, { pin = true } = {}) {
