...
const MOTION = new Set(['animating', 'fading', 'shut', 'closing', 'litein', 'liteout', 'just-open', 'sess-leaving', 'tr-enter', 'dragging', 'roll', 'settling', 'breathing']);
const SHOT_MS = 900;
...
export function morph(cur, next) {
  ...
  if (cur.nodeType === 3 || cur.nodeType === 8) {
    if (cur.nodeValue !== next.nodeValue) {
      ...
      cur.nodeValue = now;
      stats.changes++;
      if (numeric(was) && numeric(now) && cur.parentElement) rollNumber(cur.parentElement);
    }
    return true;
  }
