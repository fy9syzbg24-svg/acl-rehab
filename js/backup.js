...
// Every top level key the document holds; one missing here is dropped from a
// backup's preview and cannot be restored (audit F05, 2026-09-19). Keep it in
// step with sync/records.js.
const KNOWN = new Set(['schema', '_sync', 'settings', 'planGoals', 'planFocus', 'caseFile', 'program', 'melbourne',
  'measurements', 'mrss', 'customExercises', 'supplements', 'prnMeds', 'doses', 'days', 'pk', 'pushSubs']);
const MAPS = ['settings', 'planGoals', 'planFocus', 'caseFile', 'pushSubs'];
...
export function validateBackup(obj, { now = Date.now() } = {}) {
  ...
  for (const [parent, subs] of [['program', ['stage', 'band', 'weeklyTarget', 'days', 'clinicDays', 'mins', 'timer', 'schedule', 'seen', 'ownSteps']], ['melbourne', ['phases', 'measures']], ['pk', ['drugs', 'shots', 'feel', 'plans', 'pens']]]) {
...
export function previewImport(current, obj, { now = Date.now() } = {}) {
  ...
  const KIND = { e: 'exercise rows', d: 'days', m: 'test results', u: 'supplements', o: 'doses', n: 'as-needed medicines', s: 'settings', p: 'program choices', b: 'Melbourne marks', r: 'MRSS scores', c: 'custom exercises', g: 'plan goals', f: 'plan focus ticks', k: 'clinical history', x: 'medication records', w: 'reminder devices' };
