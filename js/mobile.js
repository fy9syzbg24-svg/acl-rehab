...
import {
  ...
  surgeryDate, saveOutstanding, reconcileBodyweight,
} from './store.js';
...
import { pageEnter, revealOnScroll, liteMotion, underHeader } from './motion.js';
...
import { renderPlayer, bindPlayer, playerLeaving, playerBusy, playerBack, refreshPlayerSafe } from './player/player.js';
...
const VIEWS = {
  ...
  // Pages of their own while the rail is up (the iPad in landscape); back
  // inside Progress the moment it is not (railpages.js).
...
/**
 * Which way a tab change moves: +1 for a tab further right in the dock, -1 for
 * further left, 0 when either view is not a tab (the player, a push from a
 * link). The order is read from the dock markup rather than repeated here, so
 * adding a tab needs no change in this file.
 */
function tabDir(from, to) {
  const names = [...document.querySelectorAll('#mtabs [data-view]')].map((b) => b.dataset.view);
...
function rawPaint(opts = {}) {
  ...
  // A rail page exists only while the rail does and only while his data holds
  // it. Rotating to portrait puts him back on the same panel inside Progress
  // rather than on a page that no longer has a way back.
  ...
  // The dock's pill (the rail's highlight on an iPad) glides to the new tab,
  // and on a phone the new icon settles (B5-4). Measured in the next frame.
  const phoneDock = !matchMedia('(min-width: 900px)').matches;
  glide(document.getElementById('mtabs'), document.querySelector(`#mtabs button[data-view="${tabView}"]:not([hidden])`), {
    target: (b) => (matchMedia('(min-width: 900px)').matches ? b : b.querySelector('svg')),
    animate: lastTab != null && tabView !== lastTab, settle: phoneDock,
  });
  lastTab = tabView;
  ...
  // A new page pushes in from the side the tab sits on; a new Progress panel
  // still fades up under its tabs (item 8, 2026-09-15, his pick). A new day or
  // week pushes the content under its header from the side he went (B5-5).
  const push = ctx.view === lastView ? ctx.pushDir || 0 : 0;
  ctx.pushDir = 0;
  if (lastView && ctx.view !== lastView) pageEnter([viewEl], tabDir(lastView, ctx.view));
...
// The custom workout timer and a finish seal are layers of their own, outside
// #modal-root (2026-09-18). A drag on the timer was being read as a pull on
// Today underneath it: its scroll was stopped and a sync fired. His report that
// day: "trying to go back and scroll, it was a little tricky to tap around".
const modalOpen = () => !!document.getElementById('modal-root')?.childElementCount
  || !!document.querySelector('.cw-back, .cel-orbit');
...
// ---- the ring in the gap (B5-4, 2026-09-23) ----
// The pull used to open an empty gap and end with nothing to see. Now a 24 px
// ring sits in the gap: its arc fills as he pulls (pullDy over the 72 px that
// counts), turns turquoise when letting go will sync, spins while it syncs
// with the content held open under it, and on success draws the green check
// with "Synced", holds 600 ms and springs back. A failed sync leaves it
// neutral (the header chip says what went wrong) and springs back. In light
// motion the arc is whole and steps in opacity (.3, .6, 1) instead of drawing.
// Everything moves by transform and opacity; it lives outside #view so the
// content's own transform never carries it.
const PULL_HOLD = 48;        // where the content waits while the sync runs
const SPIN_MIN_MS = 600;     // long enough to be seen spinning
const SYNCED_HOLD_MS = 600;  // the check stays this long before it springs back
let pullRing = null;
let pullSyncing = false;

function ring() {
  if (pullRing) return pullRing;
  pullRing = document.createElement('div');
  pullRing.className = 'pull-ring';
  pullRing.setAttribute('aria-hidden', 'true');
  pullRing.innerHTML = '<svg viewBox="0 0 24 24"><circle class="pr-track" cx="12" cy="12" r="9"/>'
    + '<circle class="pr-arc" cx="12" cy="12" r="9" pathLength="100" transform="rotate(-90 12 12)"/>'
    + '<path class="pr-check" d="M7.8 12.4l2.9 2.9 5.6-6" pathLength="14"/></svg><span class="pr-word">Synced</span>';
  document.body.appendChild(pullRing);
  return pullRing;
}

/** The ring for a content offset of `px` and a finger travel of `dy`. */
function ringTo(px, dy) {
  const r = ring();
  // Centred in the gap: the page's own top padding (about 11 px) plus the pull.
  r.style.transform = `translate(-50%, ${((px - 13) / 2).toFixed(1)}px)`;
  const f = Math.max(0, Math.min(1, dy / PULL_TRIGGER));
  const arc = r.querySelector('.pr-arc');
  if (liteMotion()) {
    arc.style.strokeDasharray = '';
    r.style.opacity = dy <= 0 ? '0' : f < 1 / 3 ? '.3' : f < 2 / 3 ? '.6' : '1';
  } else {
    arc.style.strokeDasharray = `${(f * 100).toFixed(1)} 100`;
    r.style.opacity = String(Math.min(1, px / 30).toFixed(2));
  }
  r.classList.toggle('armed', dy >= PULL_TRIGGER);
}

/** Content and ring back to rest, together, on the same spring. */
function springBack() {
  viewEl.classList.add('mspring');
  pullTo(0);
  viewEl.style.willChange = '';
  if (pullRing) {
    pullRing.classList.add('mspring');
    pullRing.style.transform = 'translate(-50%, -6.5px)';
    pullRing.style.opacity = '0';
  }
  setTimeout(() => {
    viewEl.classList.remove('mspring');
    if (pullRing && !pulling && !pullSyncing) {
      pullRing.classList.remove('mspring', 'armed', 'busy', 'done');
      pullRing.querySelector('.pr-arc').style.strokeDasharray = '';
    }
  }, 320);
}
...
function pullStop(fire) {
  ...
  if (!fire) { if (was) springBack(); return; }
  // Held open under the spinning ring while the sync runs.
  pullSyncing = true;
  viewEl.classList.add('mspring');
  pullTo(PULL_HOLD);
  const r = ring();
  r.classList.add('mspring', 'busy', 'armed');
  r.style.transform = `translate(-50%, ${((PULL_HOLD - 13) / 2).toFixed(1)}px)`;
  r.style.opacity = '1';
  holdBusy();                       // the header chip too, before the network answers
  const t0 = Date.now();
  runSync('manual').then((res) => res, () => null).then((res) => {
    paint();
    setTimeout(() => {
      r.classList.remove('busy', 'armed');
      if (res?.ok) {
        r.classList.add('done');    // green: the one colour that means Synced
        setTimeout(() => { pullSyncing = false; springBack(); }, SYNCED_HOLD_MS);
      } else {
        pullSyncing = false;        // neutral; the header chip says what happened
        springBack();
      }
    }, Math.max(0, SPIN_MIN_MS - (Date.now() - t0)));
  });
}
...
window.addEventListener('touchstart', (e) => {
  if (pullWatching || pullSyncing || e.touches.length !== 1) return;
...
// A sync pull repaints the visible view, except a running workout (B2-1): the
// data is applied at once, the player's slots are patched in place, and its full
// repaint waits for the run to stop running (refreshPlayerSafe).
function onRemote() {
  if (ctx.view === 'player' && playerBusy() && refreshPlayerSafe()) return;
  paint();
}
onRemoteChange(onRemote);
if (window.__rehabProbe) window.__rehabProbe.remote = onRemote;
// The rail comes and goes with every rotation of the iPad; so do its two pages.
watchRail(() => paint());
