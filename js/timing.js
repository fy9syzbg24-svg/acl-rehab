...
function sidesOf(item) {
  if (item?.sides === 'each') return item.firstSide === 'R' ? ['R', 'L'] : ['L', 'R'];
...
export function buildSteps(item, ex, opts = {}) {
  ...
  plan.forEach((p, i) => {
    ...
    // An open hold (item.stopwatch, 2026-09-18) does not end at its time: it
    // counts on until he taps Set done, and records what he actually held.
    // item.holdBySide: his own goal per leg for an open hold (stopwatch.js).
    const h = parseSeconds(item?.holdBySide?.[p.side]) || hold;
    if (mode === 'hold') steps.push({ ...base, kind: 'hold', secs: h || null, estSecs: h || null, hold: h, ...(item.stopwatch ? { open: true } : {}) });
 ...
 */
/**
 * How long he ACTUALLY rests on this exercise, or null (D2, 2026-09-15).
 *
 * Only for an exercise whose program prescribes no rest, where the app is
 * otherwise falling back to a flat 30 seconds that has nothing to do with him.
 *
 * A run records its TOTAL rest, not each one, so the per-rest figure is that
 * total divided by the number of rests the prescription called for. The median
 * is used rather than the mean, because one run where he answered the door
 * would drag an average and cannot move a middle value.
 *
 * It is never applied on its own. Today's row offers it and he taps to take it
 * (his rule: never invent a completion or a setting).
 */
export function learnedRest(item, ex, runs) {
  if (parseSeconds(item?.rest) > 0) return null;     // the program said so; leave it
  const sig = rxSignature(item, ex);
  const sets = num(String(sig).split('|')[1]) || 0;
  const rests = sets - 1;
  if (rests < 1) return null;                        // one set has no rest to learn
  const seen = new Set();
  const per = [];
  for (const r of (runs || []).slice().sort((a, b) => String(b.at || '').localeCompare(String(a.at || '')))) {
    ...
    const total = num(r.restSec) || 0;
    if (total > 0) per.push(total / rests);
    if (per.length >= LEARN_WINDOW) break;
  }
  if (per.length < LEARN_MIN_RUNS) return null;
  const sorted = per.slice().sort((a, b) => a - b);
  const mid = sorted.length % 2
    ? sorted[(sorted.length - 1) / 2]
    : (sorted[sorted.length / 2 - 1] + sorted[sorted.length / 2]) / 2;
  const secs = Math.round(mid / 5) * 5;              // to the nearest five seconds
  if (secs < 5 || secs > 600) return null;           // nonsense in, nothing out
  return { secs, runs: per.length };
}
...
export function minutesFor(item, ex, doc, lastMinutes = null, runs = null) {
  // Zero means untimed, his call (2026-09-19): no estimate is invented and no
  // minutes are shown, rather than the row reading "0 min".
  const o = num(doc?.program?.mins?.[item?.id]);
  if (o != null && o > 0) return { mins: o, src: 'yours' };
  if (o === 0) return { mins: null, src: 'untimed', zero: true };
...
export function fmtDayTotal(list) {
  ...
  // Non-breaking spaces hold "1h 17m" and "+ 1 untimed" together: on a phone
  // the line broke as "About 1h" / "17m + 1 untimed" (2026-09-22). Only
  // untimed work left reads "1 untimed", never "about 0 min + 1 untimed".
  const NB = '\u00a0';
  const extra = untimed ? `+${NB}${untimed}${NB}untimed` : '';
  if (!mins && untimed) return `${untimed}${NB}untimed`;
  const base = `about${NB}${fmtMins(mins).replace(/ /g, NB)}`;
