...
// The plan in use. The months above are the ORIGINAL plan, transcribed from
// the document and never edited. A revision lives in his SYNCED data
// (caseFile.plan, personal: dates, clinician goals, the show), never here, so
// the public shell stays generic. `settings.planVersion` picks which one the
// app runs on: 'original' goes back to the document, anything else uses the
// revision when there is one. These are LIVE BINDINGS, repointed by
// hydratePlan() whenever the document changes (the history.js pattern).
//
// A revision's stages have the month shape above, plus:
//   short     the one word on the tile and the ribbon
//   exMonth   the original month number whose exercises it draws on (picker)
// and the revision itself carries `meta` with its title and key dates.

export const ORIGINAL_META = {
  version: 'original',
  title: '6-Month Plan',
  label: 'Original plan',
  source: 'The six month rehab plan document, transcribed as laid out.',
  dates: [
    { label: 'Plan target: full 1-hour show', date: '2027-01-31', note: 'Month 6 also asks for two full show runs in one day by the end of January.', show: true },
  ],
};

export let PLAN_MONTHS = ORIGINAL_MONTHS;
export let PLAN_META = ORIGINAL_META;
export let PLAN_START = ORIGINAL_MONTHS[0].start;
export let PLAN_END = ORIGINAL_MONTHS[ORIGINAL_MONTHS.length - 1].end;

/** The revision in the document, if it has usable stages. */
export function revisionOf(doc) {
  const rev = doc && doc.caseFile && doc.caseFile.plan;
  return rev && Array.isArray(rev.stages) && rev.stages.length ? rev : null;
}

/** Which plan the app runs on: 'revised' or 'original'. */
export function activeVersion(doc) {
  if (!revisionOf(doc)) return 'original';
  return doc.settings?.planVersion === 'original' ? 'original' : 'revised';
}

/** The months (or stages) and meta of one version, for showing either. */
export function planOf(doc, version) {
  const rev = revisionOf(doc);
  if (version === 'revised' && rev) return { months: rev.stages, meta: { ...rev.meta, version: 'revised' } };
  return { months: ORIGINAL_MONTHS, meta: ORIGINAL_META };
}

/** Point the exports at the plan in use. */
export function hydratePlan(doc) {
  const { months, meta } = planOf(doc, activeVersion(doc));
  PLAN_MONTHS = months;
  PLAN_META = meta;
  PLAN_START = months[0].start;
  PLAN_END = months[months.length - 1].end;
}

export function monthForDate(iso, months = PLAN_MONTHS) {
  return months.find((m) => iso >= m.start && iso <= m.end) || null;
}

/** The one word for a tile or the ribbon: a stage's own, else the month. */
export const monthShort = (m) => m.short || m.monthLabel.replace(/ .*/, '').slice(0, 3);
/** The longer name: a stage's date range, else the month. */
export const monthLong = (m) => (m.short ? m.monthLabel : m.monthLabel.replace(/ .*/, ''));
