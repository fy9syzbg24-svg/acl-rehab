...
print("── Service " + "─" * 61)
lc = run("launchctl list | grep '\.acl-rehab$'")
...
print("  Motion and polish audit (2026-09-15, 989da5d to 49b2049): small changes patch in place (morph.js, rerender({ soft: true })), folds open in place (fold.js); never soft-paint a date change. README, Patching in place; FABLE-BUILD-2026-09-15.local.md.")
print("  Afternoon and evening 2026-09-15 (696a60e to edebb17): light motion at 30 frames (motion.js), five picked animations, same-day soft paints (dayKey), exercise change pushes like pages (never a crossfade), timed steps end by themselves (loop calls syncEffects), continuous: true runs an exercise as one timer, twice a day medicines suggest the next dose (gapHours). README, the sections from Light motion down; EVENING-2026-09-15.local.md.")
print("  Tests: tools/make_test_copy.py, then /dev-tests.html on the TEST server, port 8767 (430 checks: all of dev-*.js incl. dev-codex.js; no Node), tools/test_pa_import.py (41), tools/test_server_guards.py (27), tools/test_offline_update.py (39), tools/test_fable_a.py (a1 a2: 14; a3 a08 a20 a21: 33), tools/test_flows.py (workouts in real time, clocks, import), tools/qa_crawl.py (every control, add --desktop), tools/perf_motion.py (--motion full|lite; lite, reduced), tools/qa_matrix.py, tools/qa_taborder.py.")
print("  Codex audit build (2026-09-15 night): sync refuses public repos, the server needs If-Match and never deletes backups, import merges (backup.js). CODEX-AUDIT-BUILD-2026-09-15.local.md.")
